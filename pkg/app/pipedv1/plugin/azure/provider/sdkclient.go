...
type AzureClient interface {
	SetResourceTags(tags map[string]*string)
...
type sdkClient struct {
	...
	deploymentClient     *armresources.DeploymentsClient //traffic route
...
func (c *sdkClient) SetResourceTags(tags map[string]*string) {
	c.tags = tags
}
...
func (c *sdkClient) FunctionGetSCMAllowedX(ctx context.Context, resourceGroupName, functionName, slotName string) (bool, error) {
	if slotName == "" {
		...
		return *auth.CsmPublishingCredentialsPoliciesEntity.Properties.Allow, nil
	} else {
...
func (c *sdkClient) FunctionListApplicationSettingsX(ctx context.Context, resourceGroupName, functionName, slotName string) (map[string]*string, error) {
	if slotName == "" {
		...
		return resp.StringDictionary.Properties, nil
	} else {
...
func (c *sdkClient) StorageBlobSASSign(ctx context.Context, resourceGroupName, blobURL string) (string, error) {
	...
	if parsed.SAS.Encode() == "" {
		...
		if parsed.ContainerName == "" || parsed.BlobName == "" || parsed.IPEndpointStyleInfo.AccountName == "" {
...
func (c *sdkClient) DeployARMTemplate(ctx context.Context, resourceGroupName, appDir string, armTemplate config.AzureDeployTemplate) error {
	...
	if !ok {
		return fmt.Errorf("parameters field in the paramaters file %s is not formated correctly", armTemplate.DeploymentParameterFile)
	}
...
func (c *sdkClient) StorageGet(ctx context.Context, resourceGroupName, storageName string) (*armstorage.Account, error) {
	resp, err := c.accountStorageClient.GetProperties(ctx, resourceGroupName, storageName, nil)
	if err != nil {
		var respErr *azcore.ResponseError
		if !(errors.As(err, &respErr) && respErr.StatusCode == http.StatusNotFound) {
			return nil, nil
...
func (c *sdkClient) StorageFromBlobURL(ctx context.Context, resourceGroupName, blobURL string) (string, error) {
	parts, err := azblob.ParseURL(blobURL)
	if err != nil {
		return "", err
	}
	if parts.IPEndpointStyleInfo.AccountName == "" {
		return "", fmt.Errorf("url should not use ip style")
	}
	accountName := parts.IPEndpointStyleInfo.AccountName
	keysResp, err := c.accountStorageClient.ListKeys(ctx, resourceGroupName, accountName, nil)
	if err != nil {
		return "", err
	}
	if keysResp.Keys == nil || len(keysResp.Keys) == 0 {
		return "", fmt.Errorf("no keys found")
	}
	//TODO: check endpoint suffix from url
	return fmt.Sprintf("DefaultEndpointsProtocol=%s;AccountName=%s;AccountKey=%s;EndpointSuffix=core.windows.net", parts.Scheme, accountName, *keysResp.Keys[0].Value), nil
}
...
func NewAzureClient(ctx context.Context, config config.AzureDeployTargetConfig, logger sdk.StageLogPersister) (AzureClient, error) {
	client := sdkClient{
		logger: logger,
	}
	var err error
