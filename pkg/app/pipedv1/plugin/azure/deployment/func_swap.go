...
func (p *Plugin) executeAzureFuncSwapStage(ctx context.Context, dts []*sdk.DeployTarget[config.AzureDeployTargetConfig], input *sdk.ExecuteStageInput[config.AzureApplicationSpec]) sdk.StageStatus {
	...
	if needCreate {
		lp.Errorf("Sync stage should have resouce existence guaranteed")
		return sdk.StageStatusFailure
	}
	err = sdkClient.FunctionSwapSlotX(ctx, manifest.ResourceGroupName, manifest.ResourceGroupName, stageConfig.SlotName1, stageConfig.SlotName2)
	if err != nil {
